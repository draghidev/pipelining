...
2. Every modeled activation path arbitrates through the same activation-turn identity, including empty-edge publication, dispatcher provisional-turn claim, resident activation, and recovery replacement.
3. Generation identity is minted when a hand-off becomes observable and is preserved through every later claim or recovery wait.
...
| At most one activation turn | `ActivationGate`, `Pipeline` | `ActivationOccursAtMostOnce`, `ItemActivatedAtMostOnce`, `ActivationTurnNamesLiveTenure` |
| Dispatcher and final retirement pass cannot both miss a hand-off | `ActivationGate` | deadlock check over the canonical fenced protocol |
